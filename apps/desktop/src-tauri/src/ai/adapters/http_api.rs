...
impl HttpApiAdapter {
        ...
        match event_type {
            "message_start" => {
                let model = json.get("message")
                    ...
                    .unwrap_or("unknown")
                    .to_string();
                vec![AdapterEvent::SessionReady { model, tools: vec![] }]
            }
            "content_block_delta" => {
                ...
                match delta_type {
                    "text_delta" => {
                        ...
                        else { vec![AdapterEvent::StreamToken { text, phase: StreamPhase::Text }] }
                    }
                    "thinking_delta" => {
                        let text = delta.get("thinking").and_then(|v| v.as_str()).unwrap_or("").to_string();
                        if text.is_empty() { vec![] }
                        else { vec![AdapterEvent::StreamToken { text, phase: StreamPhase::Thinking }] }
                    }
                    _ => vec![]
                }
            }
            ...
            "message_stop" => {
                vec![AdapterEvent::TurnComplete { stop_reason: Some("end_turn".into()) }]
            }
            "error" => {
    ...
    fn parse_openai_sse(data: &str) -> Vec<AdapterEvent> {
        if data == "[DONE]" {
            return vec![AdapterEvent::TurnComplete { stop_reason: Some("stop".into()) }];
        }
        ...
        if let Some(reason) = finish {
            if reason != "null" {
                return vec![AdapterEvent::TurnComplete { stop_reason: Some(reason.to_string()) }];
            }
        }
        ...
        if !content.is_empty() {
            vec![AdapterEvent::StreamToken { text: content.to_string(), phase: StreamPhase::Text }]
        } else {
            vec![]
        }
    }
    ...
    fn stream_response(&self, event_tx: mpsc::Sender<AdapterEvent>) {
        ...
        thread::spawn(move || {
            ...
            let response = match req.body(body.to_string()).send() {
                Ok(r) => r,
                Err(e) => {
                    let _ = event_tx.send(AdapterEvent::Error { message: format!("HTTP error: {e}") });
                    let _ = event_tx.send(AdapterEvent::TurnComplete { stop_reason: Some("error".into()) });
                    return;
                }
            };
