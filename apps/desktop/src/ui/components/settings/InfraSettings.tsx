...
export function InfraSettings() {
  ...
  // Save a secret to the KDBX + sync to backend
  ...
  // Non-secret field update (gitlabUrl, servers)
  ...
  return (
    <div style={{ padding: "24px 32px", "max-width": "600px" }}>
      <h2 style={{ margin: "0 0 4px", "font-size": "20px", "font-weight": "600", color: "var(--text-primary)" }}>Infrastructure</h2>
      <p style={{ margin: "0 0 8px", "font-size": "13px", color: "var(--text-muted)" }}>
        Credentials stockes dans le coffre-fort chiffre (KDBX).
      </p>
      <Show when={!secrets.isUnlocked()}>
        <div style={{ padding: "12px", background: "rgba(231,76,60,0.1)", "border-radius": "var(--radius-md)", "font-size": "12px", color: "var(--danger, #e74c3c)", "margin-bottom": "24px" }}>
          Le coffre-fort n'est pas deverrouille. Les secrets ne peuvent pas etre lus.
        </div>
      </Show>
      ...
      <div style={sectionStyle}>
        ...
        <p style={subHeadingStyle}>Cles API OVH</p>
      ...
      <div style={sectionStyle}>
        ...
        <div style={fieldStyle}>
          <label style={labelStyle}>URL GitLab</label>
          ...
          <p style={helpStyle}>gitlab.com ou votre instance self-hosted.</p>
        </div>
      </div>

      {/* Servers */}
      <div style={sectionStyle}>
        <h3 style={headingStyle}>Serveurs SSH</h3>
        <div style={{ display: "flex", "flex-direction": "column", gap: "6px", "margin-bottom": "12px" }}>
          <For each={infra().servers ?? []} fallback={
            <p style={{ "font-size": "12px", color: "var(--text-muted)", padding: "8px 0" }}>Aucun serveur configure.</p>
          }>
            {(server) => (
        ...
        <Show when={!addingServer()}>
          <Button variant="secondary" size="sm" onClick={() => setAddingServer(true)}>+ Ajouter un serveur</Button>
        </Show>
        <Show when={addingServer()}>
          <div style={{ padding: "12px", background: "var(--bg-elevated)", "border-radius": "var(--radius-md)", display: "flex", "flex-direction": "column", gap: "8px" }}>
            <div style={{ display: "flex", gap: "8px" }}>
              <div style={{ flex: "1" }}><label style={labelStyle}>Nom</label><input style={inputStyle} value={newServer().label} onInput={(e) => setNewServer({ ...newServer(), label: e.currentTarget.value })} placeholder="VPS OVH" /></div>
            ...
            <div style={{ display: "flex", gap: "8px", "margin-top": "4px" }}>
              <Button variant="primary" size="sm" onClick={addServer}>Ajouter</Button>
              <Button variant="secondary" size="sm" onClick={() => setAddingServer(false)}>Annuler</Button>
            </div>
          </div>
        </Show>
      </div>
