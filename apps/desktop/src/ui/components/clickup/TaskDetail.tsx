...
export function TaskDetail() {
  const { selectedTask, closeTaskDetail, taskDetail, isLoadingTaskDetail } = useCalendarStore();

  const task = () => selectedTask() as UnscheduledTask;

  return (
    <Modal isOpen={!!selectedTask()} onClose={closeTaskDetail} title="Tache ClickUp">
      <Show when={selectedTask()}>
        <div style={{ display: "flex", "flex-direction": "column", gap: "16px" }}>
          ...
          {/* Description — loaded from ClickUp API */}
          ...
          <div style={{ display: "flex", gap: "8px", "margin-top": "4px" }}>
            <Button variant="secondary" onClick={() => openUrl(task().url)}>
              Ouvrir dans ClickUp
            </Button>
          </div>
        </div>
