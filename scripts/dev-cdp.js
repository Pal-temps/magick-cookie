#!/usr/bin/env node
// Same as dev.js but injects the WebView2 CDP debug port for Playwright testing.
...
function shutdown(commands) {
  ...
  killPorts();
  console.log("[dev:cdp] Shutdown complete.");
  process.exit(0);
}

console.log("[dev:cdp] WebView2 CDP debug port: 9222");
console.log("[dev:cdp] Playwright can connect via: http://localhost:9222");
